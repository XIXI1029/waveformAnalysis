"""
This code is designed to implement inference for the NanoDet-Plus model using OpenCV's DNN module, with the model file being nanodet_plus.onnx.
"""
import cv2
import numpy as np
import argparse
...
class my_nanodet():
    ...
    def detect(self, srcimg):
        ...
        inference_time = (end - start) / cv2.getTickFrequency()
        det_bboxes, det_conf, det_classid = self.post_process(outs)
        if len(det_bboxes) == 0:
            return srcimg, np.array([]), 0, 0
        bbox_list = []
        max_conf_idx = np.argmax(det_conf)          # 最高置信度的索引
        drawimg = srcimg.copy()
        ...
        return drawimg, bbox_list, max_conf_idx, inference_time
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgpath', type=str, default='step1/000523.jpg', help="image path")
    parser.add_argument('--input_shape', default=416, type=int, choices=[320, 416], help='input image shape')
    parser.add_argument('--modelpath', type=str, default='nanodet.onnx',
